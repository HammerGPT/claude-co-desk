    ...
    try:
        ...
        if task_data.get('executionMode') == 'immediate':
            try:
                ...
                if created_task:
                    # Build complete execution command, adding time context and work directory instruction
                    ...
                    enhanced_goal = f"{task_data['goal']} {time_context} {work_dir_instruction}"
    ...
    try:
        while True:
            ...
            if message.get('type') == 'claude-command':
                ...
                # Build complete task execution command, adding time context and work directory instruction
                time_context = get_current_time_context()
                enhanced_command = f"{command} {time_context}"
                logger.info(f"🔔 Enhanced command after time context: {enhanced_command}")
