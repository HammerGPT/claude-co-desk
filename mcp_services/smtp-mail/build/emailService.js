...
import { getDefaultSmtpConfig, getSmtpConfigs, getDefaultEmailTemplate, getEmailTemplates, logEmailActivity } from './config.js';
 ...
 */
export async function createTransport(smtpConfigId) {
    let smtpConfig;
    if (smtpConfigId) {
 ...
 */
async function generateEmailContent(templateId, templateData, subject, body) {
...
export async function sendEmail(data, smtpConfigId) {
    try {
        ...
        const { subject, body } = await generateEmailContent(data.templateId, data.templateData, data.subject, data.body);
        // Create mail options
        const mailOptions = {
            from: data.from
 ...
 */
export async function sendBulkEmails(data, smtpConfigId) {
    try {
