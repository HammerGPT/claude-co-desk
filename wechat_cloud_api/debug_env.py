...
try:
    ...
    load_dotenv('.env')
    print("Success: dotenv加载成功")
    
    for var in env_vars:
        value = os.getenv(var, 'NOT_SET')
        print(f"{var}: {value}")
except ImportError:
    print("Error: python-dotenv未安装")
...
try:
    ...
    print("Success: Settings创建成功")
    print(f"APP_ID: {settings.wechat_app_id}")
except Exception as e:
    print(f"Error: Settings创建失败: {e}")
    
print("\n=== 测试真实config.py ===")
try:
    from config import get_settings
    settings = get_settings()
    print("Success: 真实Settings创建成功")
    print(f"APP_ID: {settings.wechat_app_id}")
    print(f"PORT: {settings.port}")
except Exception as e:
    print(f"Error: 真实Settings创建失败: {e}")